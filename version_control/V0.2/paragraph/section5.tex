...
The threats and countermeasures analyzed in Sections~\ref{sec:threats} and~\ref{sec:countermeasures} have prompted a global governance response characterized by divergent jurisdictional philosophies and a persistent gap between regulatory frameworks and deployment velocity. This section examines government policies, international cooperation, and industry self-regulation.
...
Twelve frontier companies published safety frameworks by December 2025~\cite{metr2025frameworks}, sharing a common architecture of capability thresholds triggering escalating safeguards. Anthropic's RSP v3.0 defines AI Safety Levels (ASL-1 through ASL-4+)~\cite{anthropicrsp3}. OpenAI's Preparedness Framework v2.0 streamlines to ``High'' and ``Critical'' thresholds but removed persuasion risk tracking~\cite{openaiprepv2}. Google DeepMind's FSF v3.0 introduced explicit misalignment evaluation~\cite{googlefsf3}. All remain \textbf{voluntary, self-enforced, and without independent verification}.

The FLI AI Safety Index (Winter 2025) reveals that Anthropic leads (C+), followed by OpenAI (C+) and DeepMind (C); xAI, Meta, DeepSeek, and Alibaba received D/D- grades. \textbf{No company scored above D in Existential Safety}~\cite{fli2025winter}. Red teaming has matured across human expert testing~\cite{ganguli2022redteam}, automated tools (PyRIT~\cite{pyrit2024}, Garak~\cite{garak2024}), and external programs (DEF CON 31 with 2,244 hackers~\cite{defcon2023generativeredteam}).


\subsubsection{Platform Governance}
\label{sec:gov-platform}

Platforms have converged on \emph{labeling rather than removal}. Meta uses C2PA/IPTC metadata detection with evolving labels (``AI Info'') and bans AI tools for political ads~\cite{metalabeling2024}. Google deploys SynthID watermarking across modalities ($>$10B items watermarked)~\cite{synthid2025}. TikTok is the most restrictive: automatic C2PA detection, AI political ad ban, and 1.3B+ labeled videos~\cite{tiktok2023labels}. The C2PA standard (v2.2) has achieved broad hardware and platform adoption~\cite{c2pa2025}, though metadata stripping remains a fundamental challenge. Industry alliances---Partnership on AI~\cite{pai}, Frontier Model Forum~\cite{fmf2023}, White House Voluntary Commitments~\cite{whitehouse2023commitments}---provide scaffolding, but a one-year assessment found companies ``nowhere near where we need them to be''~\cite{mitreview2024assessment}.
