...
\begin{enumerate}[nosep,leftmargin=1.5em]
    ...
    \item \textbf{Instruction-data separation as a fundamental limit.} Zverev et al.\ show that all tested models fail to achieve high instruction-data separation---GPT-4 scores only 0.225 on their formal separation metric~\cite{zverev2025separation}. Both OpenAI and the UK NCSC frame prompt injection as ``unlikely to ever be fully solved.'' \emph{Open question:} Is principled instruction-data separation inherently impossible in autoregressive architectures? What alternative computational paradigms (e.g., structured query separation~\cite{chen2025struq}, declarative execution models) could enforce it?
    \item \textbf{Inverse scaling and the capability--security tension.} MCPTox reveals a positive correlation ($r$=0.64) between model capability and attack susceptibility~\cite{wang2025mcptox}---more capable models are paradoxically easier to exploit via tool poisoning. Safety alignment measurably degrades reasoning~\cite{huang2025safetytax}. \emph{Open question:} Is there a Pareto frontier beyond which capability and security are mathematically incompatible? Can alignment techniques decouple instruction-following robustness from harmful-instruction obedience?
