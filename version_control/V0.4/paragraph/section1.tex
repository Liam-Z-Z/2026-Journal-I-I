...
On the \textbf{security} dimension, generative AI has created novel attack vectors: adversarial manipulation via prompt injection and jailbreaking, data poisoning and backdoor attacks on training pipelines, supply chain vulnerabilities in open-weight model ecosystems, and---most critically---agentic exploitation, where autonomous AI agents execute multi-step attacks in real computing environments (Section~\ref{sec:cap-autonomy}). On the \textbf{safety} dimension, AIGC enables widespread deepfake production (Section~\ref{sec:threat-deepfake}), hyper-personalized disinformation (Section~\ref{sec:threat-infoop}), non-consensual intimate imagery (Section~\ref{sec:threat-ncii}), bias amplification (Section~\ref{sec:threat-bias}), and privacy leakage through training data memorization.
...
\begin{enumerate}[nosep,leftmargin=1.5em]
    ...
    \item \textbf{Attack surface evolution ladder as an analytical framework.} We formalize the escalation from direct prompt injection through jailbreaking and indirect prompt injection to agentic exploitation via the MCP ecosystem as a four-level progression characterized by three escalation dimensions (attacker access, victim agency, impact scope), providing both explanatory and predictive structure (Sections~\ref{sec:cap-autonomy} and~\ref{sec:threat-adversarial}).
