...
Static benchmarks become obsolete through saturation and contamination, while agentic safety evaluation remains primitive---no tested agent achieves a safety score above 60\%~\cite{zhang2024agentsafetybench}. MLCommons AILuminate (24,000+ prompts) explicitly notes ``negative predictive power''~\cite{ghosh2024ailuminate}: passing benchmarks does not ensure deployment safety. Contamination-proof procedural generation and formally grounded deployment-safety guarantees represent the key unsolved problems.
